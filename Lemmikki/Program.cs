...
class Program
{
    static readonly SqliteConnection conn = new("Data Source=pets.db");
    ...
    static void InitDb()
    {
        ...
        using (var cmd = new SqliteCommand(ownersTable, conn))
            cmd.ExecuteNonQuery();
        ...
        using (var cmd = new SqliteCommand(petsTable, conn))
            cmd.ExecuteNonQuery();
    }

    static void AddOwner(string name, string phone)
    {
        var cmd = new SqliteCommand("INSERT INTO owners (name, phone) VALUES (@name, @phone)", conn);
    ...
    {
        var cmd = new SqliteCommand("INSERT INTO pets (name, species, owner_id) VALUES (@name, @species, @ownerId)", conn);
        cmd.Parameters.AddWithValue("@name", name);
    ...
    {
        var cmd = new SqliteCommand("UPDATE owners SET phone = @phone WHERE id = @id", conn);
    ...
    {
        var cmd = new SqliteCommand("SELECT owners.phone FROM owners JOIN pets ON owners.id = pets.owner_id WHERE pets.name = @petName", conn);
